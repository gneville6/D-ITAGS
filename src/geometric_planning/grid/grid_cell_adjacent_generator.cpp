 ...
 */
#include "grstapse/geometric_planning/grid/grid_cell_adjacent_generator.hpp"
...
namespace grstapse
{
    GridCellAdjacentGenerator::GridCellAdjacentGenerator(const std::shared_ptr<const GridMap>& map)
        : Base({
              ...
              std::make_shared<GridEdgeApplier>(1, 1),   // NE
              std::make_shared<GridEdgeApplier>(1, -1),  // SE
              std::make_shared<GridEdgeApplier>(-1, 1),  // NW
              std::make_shared<GridEdgeApplier>(-1, -1)  // SW
          })
        , m_map(map)
    {}

    bool GridCellAdjacentGenerator::isValidNode(const std::shared_ptr<const GridCellNode>& node) const
    {
