...
namespace grstapse
{
    ...
    /**!
     * A task configuration from SE(2) that works with an ompl motion planner
     */
    class Se2OmplConfiguration : public OmplConfiguration
    {
       public:
        //! Deserializes from json
        [[nodiscard]] static std::shared_ptr<const Se2OmplConfiguration> deserializeFromJson(const nlohmann::json& j);
       ...
       protected:
        ...
        [[nodiscard]] virtual float euclideanDistance(
            const std::shared_ptr<const Se2StateOmplConfiguration>& rhs) const = 0;
    };
